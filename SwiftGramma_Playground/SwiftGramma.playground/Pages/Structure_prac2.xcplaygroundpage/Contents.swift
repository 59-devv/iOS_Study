...
func printClosestStore(currentLocation: Location, stores:[(loc: Location, name: String)]) {
    ...
    for store in stores {
        let distanceToStore = distance(current: currentLocation, target: Location(x: store.loc.x, y: store.loc.y))
...
// 현재 편의점의 위치(Tuple)
let store1 = (Location(x: 3, y: 5), name: "GS")
let store2 = (Location(x: 4, y: 6), name: "Seven7")
let store3 = (Location(x: 1, y: 7), name: "CU")
